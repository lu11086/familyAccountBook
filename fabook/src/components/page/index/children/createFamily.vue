  ...
  <div>
    ...
    <div class="createFamilyArea">
      <div class="lineTab clearfix">
        <p>家庭昵称 :</p>
        <input type="text" name="familyName" placeholder="限制10字以内；可为空；" v-model="familyName" @input="checkUnderTen" @blur="closeError"/>
        <h6 v-show="titleError" class="title">昵称不能超过10个字！</h6>
      </div>
      <div class="lineTab clearfix">
        <p>家庭简介 :</p>
        <textarea name="familyRemark" cols="30" rows="10" placeholder="限制100字以内；可为空；" v-model="familyRemark" @input="checkUnderHundred" @blur="closeError"></textarea>
      ...
      <div class="lineTab clearfix">
        ...
        <ul>
          <li class="userTab clearfix" v-for="(user, index) in userList" :key="index">
            <span class="fl">{{user}}</span><span class="fr" :class="[index == 0 ? 'noDel' : '']" @click="index == 0 || delUser(user)">删除</span>
          </li>
          <li class="userTab clearfix textIn" v-show="!userAdd">
      ...
      <transition name="opacity-fade">
        <div class="lineTab" v-show="userList.length >= 2">
          <button @click="createFamily"><i class="iconfont fabook-right-1"></i>确认创建</button>
        </div>
      </transition>
    </div>
